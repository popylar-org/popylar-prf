...
class ParameterSizeError(Exception):
    """
    Exception raised when model parameters have first dimensions with different sizes.

    Parameters
    ----------
    parameter_names: Sequence[str]
        Names of parameters that have different sizes in first dimension.
    parameter_shapes: Sequence[tuple of int]
        Shapes of parameters that have different sizes in first dimension.
    ...
    def __init__(self, parameter_names: Sequence[str], parameter_shapes: Sequence[tuple[int, ...]]):
        ...
        super().__init__(f"First dimensions of parameters {names} have different sizes: {shapes}")
...
class ResponseModel(BaseModel):
    ...
    def __call__(self, stimulus: Stimulus, parameters: pd.DataFrame) -> Tensor:
        ...
        """
        return ops.zeros_like(stimulus.grid)
