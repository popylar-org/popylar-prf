...
class BaselineAmplitude(BaseModel):
    """
    Linear baseline and amplitude model.

    Transforms a response by multiplying it with an amplitude and adding a baseline.
    ...
    def parameter_names(self) -> list[str]:
        """Names of parameters used by the model: `baseline`, `amplitude`."""
    ...
    def __call__(self, inputs: Tensor, parameters: pd.DataFrame) -> Tensor:
        ...
        ----------
        inputs : Tensor
            Input tensor with shape (num_batches, num_frames).
