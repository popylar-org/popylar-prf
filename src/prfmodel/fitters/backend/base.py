...
SGDState: TypeAlias = tuple[list, list, list, list]
...
class ParamsDict:
    ...
    def __getitem__(self, keys: list[str, Tensor]):
...
class BaseSGDFitter(keras.Model):
    ...
    @abstractmethod
    def _get_state(self) -> SGDState | None:
        pass

    @abstractmethod
    def _update_model_weights(self, x: Stimulus, y: Tensor, state: SGDState | None = None) -> tuple[dict, SGDState]:
        pass
