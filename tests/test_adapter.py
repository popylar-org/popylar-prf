...
def test_parameter_transform(transform: ParameterTransform, params_wrapper: type, params: pd.DataFrame):
    ...
    with warnings.catch_warnings():
        ...
        for param in transform.parameter_names:
            ...
            assert np.all(
                result_forward_param[np.isfinite(result_forward_param)] == ref_forward[np.isfinite(ref_forward)],
            ...
            assert np.all(
                result_inverse_param[np.isfinite(result_inverse_param)] == ref_inverse[np.isfinite(ref_inverse)],
