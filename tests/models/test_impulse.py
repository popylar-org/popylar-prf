...
class TestGammaDensity(TestIRFSetup):
    ...
    def test_gamma_density(self, frames: np.ndarray, parameters: np.ndarray):
        """Test that gamma density peaks at correct frame."""
        ...
        # The theoretical peak may not fall exactly on a sampled frame because of the
        # time resolution. Find the frame closest to the theoretical peak and assert
        # that the response's maximum occurs at that frame.
        # Handle cases where the theoretical peak lies outside the sampled frames
        # (e.g., very large peak time) by expecting the maximum at the first or
        # last frame in that case. Otherwise allow a tolerance of one resolution
        # bin because the theoretical peak may fall between sampled frames.
        expected_peak = np.where(shape < 1, 0, (shape - 1) / rate).squeeze()
        peak_resp = np.argmax(resp, axis=1)
        peak_frame = frames[peak_resp]

        for e_p, p_f, p_r in zip(expected_peak, peak_frame, peak_resp, strict=False):
            if e_p >= frames.max(axis=0):
                assert p_r == (len(frames) - 1)
            elif e_p <= frames.min(axis=0):
                assert p_r == 0
            else:
                assert abs(p_f - e_p) <= self.resolution
